...
export default function TrainingDashboard(){
 ...
 return <main>
  ...
  <section id="bank" className="panel bank-panel">
   ...
   <div className="grid">{filteredQuestions.map(x=>{const isOfficial=/^\d{3}-\d+$/.test(x.id),isAi=x.id.startsWith("ai-"),hasWorkspace=isOfficial||isAi,style=levelStyle[x.level];return <article key={x.id}><div><em>{isAi?"AI 出題":x.year}</em><span style={style?{background:style.bg,color:style.fg}:undefined}>{x.level}</span></div><h3>{x.title}{completedIds.has(x.id)&&<span className="done-badge">✓ 已完成</span>}</h3><p>{x.note}</p><footer>{hasWorkspace&&<a className="practice-link" href={`/practice/${x.id}`}>進入作答工作區</a>}{!isAi&&<a href={`/question/${x.id}`}>閱讀題目</a>}{hasWorkspace?<span className="backfill-hint">送出評測後自動回填</span>:<button onClick={()=>setActive(x)}>回填練習</button>}{!isOfficial&&<button className="delete" onClick={()=>removeQuestion(x.id,x.title)}>刪除題目</button>}</footer></article>})}</div>
