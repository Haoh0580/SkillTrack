...
export function PracticeWorkspace({ id, title, category, note, definition }: Props) {
  ...
  return <main className="practice-shell">
    <nav className="practice-nav"><Link href="/" prefetch={false} onClick={(event) => { event.preventDefault(); window.location.assign("/"); }}>← 題庫</Link><span>{category}</span><strong>{time}</strong><button onClick={() => setRunning((value) => !value)}>{running ? "暫停" : "開始計時"}</button></nav>
