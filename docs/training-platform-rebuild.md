...
2. **資料層**：題目敘述、範例測資、草稿、送出歷程的資料模型與 API。
3. **判題層**：建立 Judge Adapter；先支援一種語言與公開測資，再擴充。
...
`/api/submissions` 不負責直接跑程式；它只驗證請求、建立歷程並呼叫未來的 Judge Adapter。這能讓雲端判題服務、校內主機或其他安全沙盒可替換。
