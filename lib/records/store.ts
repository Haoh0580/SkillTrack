 ...
 * request: CREATE TABLE / CREATE INDEX use IF NOT EXISTS, and the ADD COLUMN (needed
 * because older deployments created this table before submission_id existed) is
 * wrapped to ignore the "column already exists" error on repeat runs.
 *
...
export async function ensureRecordsSchema(database: SubmissionDatabase): Promise<void> {
  await database
    ...
    .run();
  try {
    await database.prepare("ALTER TABLE records ADD COLUMN submission_id TEXT").bind().run();
  } catch (error) {
...
export async function insertFinalizedRecord(
  ...
  try {
    await database
      .prepare("INSERT INTO records (id, resource_id, score, minutes, status, notes, created_at, submission_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(crypto.randomUUID(), input.resourceId, input.score, input.minutes, input.status, input.notes, new Date().toISOString(), input.submissionId)
      .run();
    return { applied: true };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
export async function insertManualRecord(database: SubmissionDatabase, input: RecordInput): Promise<void> {
  await ensureRecordsSchema(database);
  await database
    .prepare("INSERT INTO records (id, resource_id, score, minutes, status, notes, created_at, submission_id) VALUES (?, ?, ?, ?, ?, ?, ?, NULL)")
    .bind(crypto.randomUUID(), input.resourceId, input.score, input.minutes, input.status, input.notes, new Date().toISOString())
    .run();
}
